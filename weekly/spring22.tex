...
\begin{document}
	...
	\begin{frame}{LBM Code}
		...
		Motivation: 
		\begin{itemize}
			\item 3D version for arbitrary porous media and boundary conditions
			\item Parallelization
			\item Possible coupling with other transport equations
		\end{itemize}
		...
		\begin{itemize}
			...
			\item LBM Functions (Parameters - Coll. Operators - Steps) 
	...
	\begin{frame}{Validations}
		...
		The main validation sources have been: analytical solutions, Cheng's codes, and a priori physics understanding (qualitatively).\\
	...
	\begin{frame}{Single phase validations (qualitative)}
		\begin{figure}[h]
			...
			\begin{subfigure}{.5\textwidth}
				...
				\caption{Arbitrary porous medium (from real imaging).}
				\label{fig:sub1}
			\end{subfigure}%
			\begin{subfigure}{.5\textwidth}
				\centering
	...
	\begin{frame}{Ongoing work}
		\begin{itemize}
			...
			\item Generalizing the Shan-Chen force at boundary blocks
			\item Viscosity per phase, to reproduce falling and raising droplet/bubble within LBM velocity ranges (see falling droplet video).
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Questions}
		\begin{itemize}
			...
			\begin{split}
				...
				f_{i,\text{in}}^{\text{bndry}} = f_{\bar{i},\text{in}}^{\text{bndry}} - g(\mathbf{u}_{n}^{\text{neighbor}})
			\end{split}
			\end{equation*}
			...
			\item<3> Is there a general treatment for corners, no matter what the BCs are?
			\item<3> Pseudopotential per component. Was this tried and what were the difficulties?
			\item<3> Reason for velocity modification, and which $\tau$ to use in two-phase systems?
			\begin{equation}
			\mathbf{u}^{mod} = \mathbf{u} + \frac{\beta \mathbf{F}}{(\tau - 0.5)\psi^2}
			\end{equation}
		\end{itemize}
	...
	\begin{frame}{\href{https://www.overleaf.com/project/5f5cfacb13ed640001eeb63d}{\color{green}Paper}}
		...
		Which are the cases initially thought to serve as validation?\\~\\
		
		Current ideas/tasks for dynamic validations:
		\begin{itemize}
			\item Young-Laplace equation
			\item Identify analytical expression for oscillation frequency
			\item Rayler-Taylor instability (surface waves)
			\item Rising bubble/falling droplet (easy analytical solution!)
