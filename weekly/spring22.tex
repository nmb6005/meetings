...
\begin{document}
	...
	\begin{frame}{Interface thermodynamics}
		\begin{itemize}
			\item Is $\sigma = \frac{d \mathbf{F}}{d \mathbf{l}}$ a directional property for a given geometrical setup\footnote{See slide 9, "surface tension forces"}? Rotates direction of $d \mathbf{l}$\footnote{Force is tangential to interface and normal to the perimeter.}. I can not see how $\sigma$ is isotropic when tension is not coming from all directions. 
			\item If $\sigma$ is a macroscopic property, how can we determine its uniformity across\footnote{Is it more appropriate to use along?} the interface
			...
			\item Slide 11. If $dl$ goes along the perimeter, what it means $dl \sin \alpha$? Again, the tensor nature of $\sigma$ confuses me. After solving this, I would say it is more appropriate an integration along $\pi$ with less terms. Slide 12. Missing dot product between $\mathbf{e_n} \cdot d\mathbf{S}$? Does this justify avoiding directionality of $F_n$? Possible typo about "this downward". 
			
			\item If Laplace's internal pressure $K$ is the cohesive pressure $p_\text{coh} = a/\bar{v}$, and internal pressure is $K + \sigma k_t$, why this does not match?:
			
			\begin{equation*}
				\begin{split}
				p_i = p_o + \sigma k_t\\
				\frac{RT}{\bar{v}_i-b} - \frac{a}{\bar{v}_i^2} = \frac{RT}{\bar{v}_o-b} - \frac{a}{\bar{v}_o^2} + \sigma k_t\\
				\frac{RT}{\bar{v}_i-b} - K_i = \frac{RT}{\bar{v}_o-b} - K_o + \sigma k_t
				\end{split}
			\end{equation*}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{Interface thermodynamics}
		\begin{itemize}
			\item In PNG 520, $p = p_k - p_{att}$. However, redefined in the slides $p = p_k - p_{coh}$, where $p_{coh} = p_{att} + p_{rep}$. Are these repulsion through potentials but not necessarily to small separations (see slide 21)? What it means $p_{coh}>> p $ in liquids ($p_{coh} >>> p_k/2$)?
			
			\item Would help to add Helmholtz equation in slides 19 and 21.
			
			\item I still have not this clear: is $\sigma dA$ an irreversible or a reversible term? In slide 25, write that invariability of curvature with deformation will be explained further (last sentence).
	...
	\begin{frame}[t]{Interface thermodynamics}
		\begin{itemize}
			...
			\item Slide 51. Typo $v^L$ instead of $\mu^L$. Say before taking the differential, that density ratio and $\sigma$ variations will be neglected. Why is it better to assume incompressibility than ideality + incompressibility? Is it better because we are putting the correct vapor density? Is not explicit in T. 
